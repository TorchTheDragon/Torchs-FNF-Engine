...
class PhillyStreetsErect extends BaseStage {
	...
	override function create()
	{
		...
		var grey:BGSprite = new BGSprite('phillyStreets/erect/greyGradient', 300, 200, 1, 1);
		add(grey);
		...
		if(!ClientPrefs.data.lowQuality)
		{
			...
			mtb = new FlxBackdrop(Paths.image("phillyStreets/erect/mistBack"), X);
        	mtb.setPosition(0, 215);
        	mtb.blend = ADD;
        	mtb.scrollFactor.set(0.9, 0.9);
        	mtb.velocity.x = 30;
			mtb.alpha = 0.7;
        	mtb.antialiasing = ClientPrefs.data.antialiasing;
        	add(mtb);

			mtm = new FlxBackdrop(Paths.image("phillyStreets/erect/mistMid"), X);
        	mtm.setPosition(0, 205);
        	mtm.blend = ADD;
        	mtm.scrollFactor.set(0.9, 0.9);
			mtm.alpha = 0.7;
        	mtm.velocity.x = -20;
        	mtm.antialiasing = ClientPrefs.data.antialiasing;
        	add(mtm);
		}
		...
		mtf = new FlxBackdrop(Paths.image("phillyStreets/erect/mistMid"), X);
        mtf.setPosition(0, 350);
        mtf.blend = ADD;
        mtf.scrollFactor.set(0.9, 0.9);
		mtf.alpha = 0.7;
        mtf.velocity.x = 30;
        mtf.antialiasing = ClientPrefs.data.antialiasing;
