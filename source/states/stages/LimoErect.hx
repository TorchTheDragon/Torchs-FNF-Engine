...
class LimoErect extends BaseStage
{
	...
	// event
	var limoKillingState:HenchmenKillStateErect = WAIT;
	...
	override function create()
	{
		...
		skyBG = new BGSprite('limo/erect/limoSunset', -120, -200, 0.1, 0.1);
		...
		if(!ClientPrefs.data.lowQuality) {
			...
			limoCorpseTwo = new BGSprite('gore/noooooo', -500, limoMetalPole.y - 130, 0.4, 0.4, ['henchmen death'], true);
			add(limoCorpseTwo);
				...
				case KILLING:
					...
					for (i in 0...dancers.length) {
						if(dancers[i].x < FlxG.width * 1.5 && limoLight.x > (370 * i) + 170) {
							switch(i) {
								case 0 | 3:
									if(i == 0) FlxG.sound.play(playWeekSound('dancerdeath'), 0.5);
	...
	{
		//trace('Car drive');
		FlxG.sound.play(randomWeekSound('carPass', 0, 1), 0.7);
