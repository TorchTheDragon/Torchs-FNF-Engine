...
class NoteSplash extends FlxSprite
{
	...
	public function spawnSplashNote(?x:Float = 0, ?y:Float = 0, ?noteData:Int = 0, ?note:Note, ?randomize:Bool = true, ?skin:String = null, ?library:String = null)
	{
		...
		else if (PlayState.isPixelStage || note != null && note.isNotePixel()) rgbShader.pixelAmount = 6;
...
class PixelSplashShaderRef 
{
	...
	public function changeRGB(colors:Array<FlxColor>) { // I forgot shaders use a 0-1 scale and not a 0-255 scale
	...
	public function new(?isPixelNote:Bool = false) {
		reset();
		enabled = true;

		if (!PlayState.isPixelStage || !isPixelNote) pixelAmount = 1;
