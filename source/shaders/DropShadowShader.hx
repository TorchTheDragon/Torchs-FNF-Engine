...
class DropShadowShader extends FlxShader
{
  ...
  @:glFragmentSource('
      ...
      uniform sampler2D shadowIntensityMap; // Nueva textura opcional
      uniform sampler2D thresholdMap; // Nueva textura opcional

      float localThreshold = thr;
#ifdef USE_THRESHOLD_MAP
localThreshold = flixel_flixel_texture2D(thresholdMap, openfl_TextureCoordv).r; // O el canal que prefieras
#endif

      // Usar localThreshold en vez de thr
      ...
      vec3 createDropShadow(vec3 col, float curThreshold, bool useMask) {
        ...
        #ifdef USE_INTENSITY_MAP
        shadowIntensity = flixel_texture2D(shadowIntensityMap, openfl_TextureCoordv).r; // O el canal que prefieras
        #endif

        col.rgb += dropColor.rgb * ((1.0 - (dropShadowAmount * str))*intensity*shadowIntensity);
